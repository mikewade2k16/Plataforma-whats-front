...
export function getAuthConfig() {
    ...
    return {
        name: config.authCookieName as string,
        secure: !!config.authCookieSecure,
        sameSite: (config.authCookieSameSite || 'lax') as 'lax' | 'strict' | 'none',
        maxAge: Number(config.authCookieMaxAge || 3600),
        baseURL: config.laravelBaseUrl as string, // <- Agora correto
    }
}
