// server/api/admin/users/batch.post.ts
...
export default defineEventHandler(async (event) => {
    ...
    for (const op of ops) {
        try {
            if (op.type === 'create') {
                const res = await proxyToLaravel(event, '/api/admin/users', { method: 'POST', body: op.payload })
                results.push({ id: op.id, ok: true, data: res }); continue
            }
            if (op.type === 'update') {
                const { id, patch } = op.payload
                const res = await proxyToLaravel(event, `/api/admin/users/${id}`, { method: 'PUT', body: patch })
                results.push({ id: op.id, ok: true, data: res }); continue
            }
            if (op.type === 'delete') {
                const { id } = op.payload
                let okData: any = null
                try {
                    okData = await proxyToLaravel(event, `/api/admin/users/${id}`, { method: 'DELETE' })
                } catch (e: any) {
